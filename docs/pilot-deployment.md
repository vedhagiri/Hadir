# Hadir — pilot deployment runbook
...
| --- | --- | --- |
| `HADIR_DATABASE_URL` | App-runtime Postgres URL (`hadir_app` role) | `postgresql+psycopg://hadir_app:hadir_app@postgres:5432/hadir` |
| `HADIR_ADMIN_DATABASE_URL` | Migrations + seed-script URL | `postgresql+psycopg://hadir:hadir@postgres:5432/hadir` |
| `HADIR_APP_DB_PASSWORD` | Set by 0001 migration on the `hadir_app` role | change for shared envs |
| `HADIR_ADMIN_DB_PASSWORD` | Same for `hadir_admin` | change for shared envs |
| `HADIR_SESSION_SECRET` | Reserved (sessions are opaque tokens; not used to sign yet) | random 64-byte URL-safe |
| `HADIR_FERNET_KEY` | Encryption key for face crops, capture crops, embeddings, RTSP URLs | **generate per deployment**: `python3 -c "from cryptography.fernet import Fernet; print(Fernet.generate_key().decode())"` |
| `HADIR_TENANT_MODE` | Pilot is `single`; v1.0 SaaS is `multi` | `single` |
| `HADIR_ENV` | `dev` exposes the `/api/_test/*` smoke endpoints; **production must be `production`** (or anything but `dev`) | `production` for Omran handoff |
| `HADIR_SESSION_IDLE_MINUTES` | Sliding session timeout | `60` |
| `HADIR_LOGIN_MAX_ATTEMPTS` | Pilot rate limiter ceiling | `10` |
| `HADIR_LOGIN_RATE_LIMIT_RESET_MINUTES` | Pilot rate limiter reset interval | `10` |
| `HADIR_MATCH_THRESHOLD` | Cosine cutoff (hard); below → unidentified | `0.45` |
| `HADIR_LOCAL_TIMEZONE` | IANA name; UTC→local conversion for shift policy | `Asia/Muscat` |
| `HADIR_ATTENDANCE_RECOMPUTE_MINUTES` | APScheduler cadence for today's recompute | `15` |

> ⚠ `HADIR_FERNET_KEY` is the single key for **all** at-rest encryption
...
`hadir_admin` + `hadir_app` Postgres roles, all 12 application tables,
...
```bash
docker compose exec -e HADIR_SEED_PASSWORD='choose-a-strong-password' backend \
...
`HADIR_MATCH_THRESHOLD` is hard. The matcher logs the top-3 scores per
...
- `HADIR_ENV` MUST NOT be `dev` in production. The dev-only test
  endpoints under `/api/_test/*` (used by `frontend/tests/pilot-smoke.spec.ts`)
  are mounted only when `HADIR_ENV=dev` and would let any
...
- `HADIR_FERNET_KEY` MUST be unique per deployment and stored in the
  customer's secret manager.
- The `hadir_app` role must be the runtime DB user. `hadir_admin` is
