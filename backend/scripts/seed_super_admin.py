...
Or via env vars (CI / deploy)::
    ...
    HADIR_SUPER_ADMIN_EMAIL=you@mts.om \\
    HADIR_SUPER_ADMIN_PASSWORD='…' \\
    HADIR_SUPER_ADMIN_FULL_NAME='Your Name' \\
...
The script connects via ``hadir_admin`` (the migrations role) so it
can write to ``public.mts_staff`` even on a brand-new DB before the
``hadir_app`` grants are exercised by any other path.
"""
...
logger = logging.getLogger("hadir.seed_super_admin")
...
def _parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--email",
        default=os.environ.get("HADIR_SUPER_ADMIN_EMAIL"),
        help="Operator email. Defaults to $HADIR_SUPER_ADMIN_EMAIL.",
    )
    parser.add_argument(
        "--password",
        default=None,
        help=(
            "Operator password. If omitted, reads $HADIR_SUPER_ADMIN_PASSWORD."
        ),
    )
    parser.add_argument(
        "--full-name",
        default=os.environ.get("HADIR_SUPER_ADMIN_FULL_NAME", "MTS Super Admin"),
        help="Display name. Defaults to $HADIR_SUPER_ADMIN_FULL_NAME or 'MTS Super Admin'.",
    )
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    if not args.email:
        logger.error(
            "No email supplied. Pass --email or set HADIR_SUPER_ADMIN_EMAIL."
        )
        return 2
    email = args.email.strip().lower()

    password = args.password or os.environ.get("HADIR_SUPER_ADMIN_PASSWORD")
    if not password:
        logger.error(
