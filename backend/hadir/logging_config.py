...
* ``backend/logs/audit.log`` — a *separate* logger named
  ``hadir.audit`` carries the application-side audit
...
DEFAULT_LOG_DIR = Path(os.environ.get("HADIR_LOG_DIR", "/app/logs"))
APP_LOG_NAME = "app.log"
AUDIT_LOG_NAME = "audit.log"

# Daily rotation, 30 backups kept (matches BRD §"Logs"
# retention). Operators tighten via ``HADIR_LOG_BACKUP_COUNT``
# without code changes.
DEFAULT_BACKUP_COUNT = int(os.environ.get("HADIR_LOG_BACKUP_COUNT", "30"))
...
class GzipRotatingFileHandler(logging.handlers.TimedRotatingFileHandler):
    ...
    plain copy. ``gzip`` saves ~80% on a typical Hadir log
...
def configure_logging(
    *,
    ...
    if enable_files is None:
        ...
        # ``HADIR_LOG_DISABLE_FILES`` env var the conftest sets.
        enable_files = (
            os.environ.get("HADIR_LOG_DISABLE_FILES", "0") not in ("1", "true", "True")
        )
    ...
    # the other.
    audit = logging.getLogger("hadir.audit")
...
def audit_logger() -> logging.Logger:
    ...
    return logging.getLogger("hadir.audit")
