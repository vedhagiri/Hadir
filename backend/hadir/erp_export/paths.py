...
``{HADIR_ERP_EXPORT_ROOT}/{tenant_id}/`` — the load-bearing P19 red
...
``hadir-attendance-{ts}.{ext}`` filename when the resolved path is a
directory.
"""
...
def tenant_root(*, tenant_id: int) -> Path:
    """Return ``{HADIR_ERP_EXPORT_ROOT}/{tenant_id}`` as an absolute Path."""
