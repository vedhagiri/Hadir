...
2. **Matching** — on each capture detection, compute cosine similarity
   ...
   ``HADIR_MATCH_THRESHOLD`` (default 0.45). Below threshold → stay
...
and the enrollment helpers used by ``hadir.employees.router``.
"""

from hadir.identification.matcher import matcher_cache
from hadir.identification.router import router
