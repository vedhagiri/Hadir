...
lookup to validate, and rotating ``HADIR_REPORT_SIGNED_URL_SECRET``
...
(``HADIR_REPORT_SIGNED_URL_RATE_LIMIT_PER_MINUTE``). The endpoint
...
def build_download_url(*, base_url: str, run_id: int, token: str) -> str:
    ...
    ``HADIR_OIDC_REDIRECT_BASE_URL`` since that's what the operator
