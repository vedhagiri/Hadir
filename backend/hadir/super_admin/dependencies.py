...
``current_super_admin`` validates the ``hadir_super_session`` cookie,
...
Distinct from ``hadir.auth.dependencies.current_user`` — Super-Admins
...
from hadir.db import get_engine, mts_staff
...
def current_super_admin(
    request: Request,
    response: Response,
    hadir_super_session: str | None = Cookie(default=None, alias=SUPER_SESSION_COOKIE),
) -> CurrentSuperAdmin:
    ...
    with engine.begin() as conn:
        session_row: SuperSessionRow | None = load_session(conn, hadir_super_session)
