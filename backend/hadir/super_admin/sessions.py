...
Modelled on ``hadir.auth.sessions`` but stored in
...
The cookie used by the request middleware is ``hadir_super_session``
(distinct from the tenant cookie ``hadir_session``) so a single browser
...
from hadir.db import super_admin_sessions

# Same entropy as tenant sessions.
_TOKEN_BYTES = 48

# Cookie name used by the Super-Admin login flow + the request
# middleware. Kept distinct from ``hadir_session`` so both can coexist
# in one browser without collision.
SUPER_SESSION_COOKIE = "hadir_super_session"
