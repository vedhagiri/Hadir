...
1. **Super-Admin session with active impersonation** (P3): the
   ``maugood_super_session`` cookie maps to a row in
...
3. **Tenant session** (P1): ``maugood_session`` cookie maps to
...
ContextVar themselves via ``maugood.db.tenant_context``.
"""
...
from maugood.auth.audit import (
...
from maugood.tenants.scope import resolve_tenant_schema
...
class TenantScopeMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(
        ...
        *,
        ...
        super_cookie_name: str = "maugood_super_session",
        tenant_cookie_name: str = "maugood_tenant",
    ) -> None:
        super().__init__(app)
    ...
    async def dispatch(self, request, call_next):  # type: ignore[no-untyped-def]
        ...
        try:
            ...
            from maugood.db import reset_tenant_schema  # noqa: PLC0415
    ...
    def _resolve(
        ...
        *,
        ...
        # 2. Tenant session, scoped by the ``maugood_tenant`` cookie when
    ...
    def _resolve_tenant(
        ...
        engine = get_engine()
        # P5: scope the session lookup by the ``maugood_tenant`` cookie
