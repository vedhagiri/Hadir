...
(``maugood_super_session``) and a separate dependency
...
from maugood.detection.detectors import _detect_lock
            ...
            try:
                with tenant_context(str(r.schema_name)):
                    with admin_engine.begin() as conn:
                        ...
                        from maugood.db import cameras as cameras_table  # noqa: PLC0415
...
def _scheduled_jobs() -> list[ScheduledJobOut]:
    ...
    try:
        from maugood.attendance import attendance_scheduler  # noqa: PLC0415

        candidates.append(("attendance_recompute", attendance_scheduler))
    except Exception:  # noqa: BLE001
        pass
    try:
        from maugood.scheduled_reports import report_runner  # noqa: PLC0415

        candidates.append(("scheduled_reports", report_runner))
    except Exception:  # noqa: BLE001
        pass
    try:
        from maugood.notifications import notification_worker  # noqa: PLC0415

        candidates.append(("notifications_worker", notification_worker))
    except Exception:  # noqa: BLE001
        pass
    try:
        from maugood.retention import retention_scheduler  # noqa: PLC0415

        candidates.append(("retention", retention_scheduler))
    except Exception:  # noqa: BLE001
        pass
    try:
        from maugood.employees.lifecycle_cron import lifecycle_scheduler  # noqa: PLC0415
