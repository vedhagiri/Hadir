...
use a distinct cookie (``maugood_super_session``) — a single browser can
...
from maugood.db import get_engine, mts_staff, tenant_context, tenants
from maugood.super_admin.audit import write_super_admin_audit
from maugood.super_admin.dependencies import CurrentSuperAdmin, current_super_admin
from maugood.super_admin.repository import (
    TenantDetail,
    TenantSummary,
    get_tenant_detail,
    list_tenants,
...
# Re-exported from the central ``maugood.tenants.slug`` module so call
# sites stay in sync if the rule ever moves.
from maugood.tenants.slug import SLUG_RE as _SLUG_RE  # noqa: E402
