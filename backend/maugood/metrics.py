...
Single source of truth for every metric Maugood emits. Importers
mutate the module-level objects below; the FastAPI app exposes
them at ``/metrics`` (internal-only — nginx in
``ops/nginx/maugood.conf.template`` does NOT proxy this path).
...
maugood_capture_frames_total = Counter(
    "maugood_capture_frames_total",
...
maugood_detection_events_total = Counter(
    "maugood_detection_events_total",
...
#    background tick — see ``maugood.metrics.refresher``.
maugood_active_sessions = Gauge(
    "maugood_active_sessions",
...
def observe_capture_frame(tenant_id: int | None, camera_id: int | None) -> None:
    maugood_capture_frames_total.labels(
...
def observe_detection_event(
    ...
    maugood_detection_events_total.labels(
...
def set_camera_reachable(
    ...
    maugood_camera_reachable.labels(
        tenant=_safe_label(tenant_id), camera=_safe_label(camera_id)
...
def observe_attendance_recomputed(tenant_id: int | None, count: int) -> None:
    if count <= 0:
        return
    maugood_attendance_records_computed_total.labels(
        tenant=_safe_label(tenant_id)
    ).inc(count)
...
def observe_scheduler_failure(
    ...
    maugood_scheduler_jobs_failed_total.labels(
...
def observe_email_send(
    ...
    maugood_email_send_total.labels(
        tenant=_safe_label(tenant_id),
...
def set_active_sessions(tenant_id: int | None, count: int) -> None:
    maugood_active_sessions.labels(tenant=_safe_label(tenant_id)).set(
        float(count)
    )
...
def install_scheduler_failure_listener(
    ...
    increments ``maugood_scheduler_jobs_failed_total``.
