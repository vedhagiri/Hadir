...
DO NOT mount these in production. ``maugood.main.create_app`` refuses to
attach this router unless ``MAUGOOD_ENV == 'dev'`` — see also
...
from maugood.db import cameras, detection_events, employees, get_engine
...
def tick_metrics(user: Annotated[CurrentUser, ADMIN]) -> TickMetricsResponse:
    """P26 dev-only: bump every Maugood Prometheus counter once
