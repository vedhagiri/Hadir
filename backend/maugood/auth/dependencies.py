...
def current_user(
    ...
    Also sets ``request.state.tenant_id`` so ``maugood.tenants.scope`` picks
    ...
    won't exist (the operator is logged in via ``maugood_super_session``),
    ...
    ``maugood.super_admin.audit.write_audit_dual`` (the per-tenant
    handlers detect impersonation on ``request.state``).
    """

    if not maugood_session:
    ...
    with engine.begin() as conn:
        session_row: SessionRow | None = load_session(conn, maugood_session)
