...
* ``GET /api/auth/oidc/callback`` — handles Entra's redirect back.
  ...
  and either creates a Maugood session or refuses.
...
STATE_COOKIE_NAME = "maugood_oidc_state"
...
# to a Microsoft tenant, not to a Maugood tenant, so several Maugood tenants
...
def oidc_callback(
    ...
    maugood_oidc_state: Optional[str] = Cookie(default=None, alias=STATE_COOKIE_NAME),
) -> Response:
