...
def _env_file_recorder_path() -> Optional[str]:
    """If ``MAUGOOD_EMAIL_RECORDER_PATH`` is set, every send writes a
    ...
    return os.environ.get("MAUGOOD_EMAIL_RECORDER_PATH") or None
...
def get_sender(config: SenderConfig) -> EmailSender:
    ...
    The dev-only ``MAUGOOD_EMAIL_RECORDER_PATH`` env var is the
