...
from maugood.emailing.render import render_report_email_html
from maugood.emailing.secrets import decrypt_secret
from maugood.reporting.attendance import build_xlsx
from maugood.reporting.pdf import HEX_PALETTE, build_pdf
from maugood.scheduled_reports.signed_url import build_download_url, make_token
...
def _build_report_for_schedule(
    ...
    from maugood.attendance.repository import local_tz_for  # noqa: PLC0415
    ...
    if fmt == "xlsx":
        ...
        data, rows = build_pdf(
            ...
            department_ids=department_ids,
            employee_id=int(employee_id) if employee_id else None,
            generated_by_email="scheduled-runner@maugood",
        )
    ...
    filename = (
        f"maugood-attendance-{tenant_slug}-{start.isoformat()}"
...
def _tick() -> int:
    ...
    try:
        from maugood.erp_export.runner import tick_due_exports  # noqa: PLC0415
    ...
    # watcher itself.
    try:
        from maugood.notifications.camera_watch import (  # noqa: PLC0415
