...
1. ``check_production_config`` — fail-fast at app startup when
   ``MAUGOOD_ENV=production`` and the config isn't HTTPS-ready
...
3. ``SecurityHeadersMiddleware`` — adds HSTS, X-Frame-Options,
   ...
   ``docker/nginx/maugood.conf`` ships the same headers — keeping
   them on the backend too means the API is safe even if
   somebody fronts it with a different proxy.

The middleware is mounted in ``maugood.main.create_app`` only when
...
class ProductionConfigError(RuntimeError):
    """Raised when ``MAUGOOD_ENV=production`` boots with unsafe config."""
...
def check_production_config(settings: Settings) -> None:
    ...
    if not settings.session_cookie_secure:
        failures.append(
            "MAUGOOD_SESSION_COOKIE_SECURE must be 'true' in production "
            "(session cookies will not survive an HTTPS-only client)"
        )

    if not settings.behind_proxy:
        failures.append(
            "MAUGOOD_BEHIND_PROXY must be 'true' in production "
    ...
    if not settings.allowed_origins:
        failures.append(
            "MAUGOOD_ALLOWED_ORIGINS must list at least one origin in "
            "production (e.g. https://maugood.example.com)"
        )

    base = (settings.oidc_redirect_base_url or "").lower()
    if base and not base.startswith("https://"):
        failures.append(
            f"MAUGOOD_OIDC_REDIRECT_BASE_URL must start with https:// in "
    ...
    for attr, placeholder in _PLACEHOLDER_SECRETS.items():
        ...
        if value == placeholder:
            failures.append(
                f"MAUGOOD_{attr.upper()} is still set to the dev placeholder; "
    ...
    if failures:
        ...
        raise ProductionConfigError(
            "Refusing to start: MAUGOOD_ENV=production but the following "
...
#                      the ``maugood-internal`` network only;
