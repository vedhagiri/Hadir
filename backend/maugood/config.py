"""Pydantic Settings for the Maugood backend.
...
class Settings(BaseSettings):
    ...
    All variables are prefixed with `MAUGOOD_` so they don't collide with other
    ...
    # Two connection URLs — the app runs as `maugood_app` (limited grants,
    # insert/select only on audit_log) and migrations/admin scripts run as
    # `maugood_admin` (full owner rights). See backend/CLAUDE.md "Database
    # roles and grants" for the policy this enforces.
    database_url: str = Field(
        default="postgresql+psycopg://maugood_app:maugood_app@postgres:5432/maugood",
        description="App-runtime SQLAlchemy URL. Pilot schema is `main`.",
    )
    admin_database_url: str = Field(
        default="postgresql+psycopg://maugood:maugood@postgres:5432/maugood",
    ...
    session_cookie_name: str = "maugood_session"
    ...
    # Comma-separated list of origins (``https://maugood.example.com``) the
    ...
    allowed_origins_raw: str = Field(default="", alias="MAUGOOD_ALLOWED_ORIGINS")
