...
from maugood.auth.sessions import is_expired, load_session, touch_session
from maugood.config import get_settings
from maugood.capture import capture_manager
from maugood.capture.event_bus import event_bus
...
def _ws_authorise(
    ...
    session_id = websocket.cookies.get("maugood_session")
    if not session_id:
        return None
    tenant_cookie = websocket.cookies.get("maugood_tenant") or "main"
    engine = get_engine()
    from maugood.auth.dependencies import (  # noqa: PLC0415
...
def events_csv(
    ...
    filename = (
        f"maugood-events-{camera_id}-{datetime.now(tz=timezone.utc).strftime('%Y%m%d-%H%M%S')}.csv"
    )
    return StreamingResponse(
