...
class _StubAnalyzer:
    ...
    def detect_and_count(self, frame) -> "tuple[list[Detection], int, list]":
        """Delegates to ``detect`` for face detections; person_count=0
        + empty person_boxes so existing tests remain purely
        face-detection driven."""
    ...
    def detect_person_boxes(self, _frame) -> list:
        """Stub returns empty — capture tests don't exercise the
        persons-only preview overlay."""
        return []
...
def test_per_tenant_config_changes_isolated(admin_engine) -> None:
    ...
    try:
        ...
        synthetic = _FakeWorker(camera_id=1)
...
def test_is_capture_fresh_tracks_last_frame_at(admin_engine) -> None:
    """Camera liveness tracks the reader's _last_frame_at. There is no
    preview/viewer subsystem any more — liveness is purely reader-side."""
    w = CaptureWorker(
        engine=get_engine(),
        scope=TENANT,
        camera_id=4242,
        camera_name="liveness-test",
        rtsp_url_plain="rtsp://fake/x",
    ...
    w._last_frame_at = time.time() - 100.0
    assert w.is_capture_fresh(max_age_s=10.0) is False
